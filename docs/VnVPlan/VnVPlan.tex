...
\begin{enumerate}
  ...
  \item{EPET-01: \textbf{Function Under Adverse Environmental Conditions}}
  
  Type: Static, Manual

  How test will be performed: Pot-pulator will run continuously for 8 hours in an environment with aerial pollution similar to that expected during 
  operation at Sheridan Nursery. A tester will be present to ensure pots and trays are available to the machine at all times so operation is never 
  interrupted. Behaviour will be observed to ensure machine is able to function under long-term exposure to an environment with aerial pollution.
